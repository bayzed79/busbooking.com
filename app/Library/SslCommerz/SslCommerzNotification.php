...
class SslCommerzNotification extends AbstractSslCommerz
{
    ...
    public function makePayment(array $requestData, $type = 'checkout', $pattern = 'json')
    {
        ...
        // Check if formattedResponse is valid
        if (!is_array($formattedResponse) || empty($formattedResponse)) {
    ...
    public function setRequiredInfo(array $info)
    {
        ...
        if (!isset($info['total_amount']) || $info['total_amount'] === null) {
            throw new \Exception('total_amount is required and cannot be null');
        }
        if (!isset($info['currency']) || $info['currency'] === null) {
            throw new \Exception('currency is required and cannot be null');
        }
        if (!isset($info['tran_id']) || $info['tran_id'] === null) {
            throw new \Exception('tran_id is required and cannot be null');
        }
        if (!isset($info['product_category']) || $info['product_category'] === null) {
            throw new \Exception('product_category is required and cannot be null');
        }
