    ...
    public function formatResponse($response, $type = 'checkout', $pattern = 'json')
    {
        ...
        if ($type != 'checkout') {
            ...
            if (!empty($sslcz['GatewayPageURL'])) {
                ...
                $storeLogo = isset($sslcz['storeLogo']) ? $sslcz['storeLogo'] : '';
                
                if(!empty($this->getApiUrl()) && $this->getApiUrl() == 'https://securepay.sslcommerz.com') {
                   $response = json_encode(['status' => 'SUCCESS', 'data' => $sslcz['GatewayPageURL'], 'logo' => $storeLogo]);
                } else {
                    $response = json_encode(['status' => 'success', 'data' => $sslcz['GatewayPageURL'], 'logo' => $storeLogo]);
                }
            } else {
                // Check if failedreason exists before accessing it
                $failedReason = isset($sslcz['failedreason']) ? $sslcz['failedreason'] : 'Unknown error';
